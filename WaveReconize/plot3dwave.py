...
for ii in range(3):
    seismic3d[:,ii]=signal.detrend(stream[ii].data[int(nzs_t1[ii]):int(nzs_t1[ii])+nzpt])
la=31.0
lo=102.36
...
for itr in dt1:
    ...
    for jj in range(shape[0]):
        ...
        ons=np.ones([shape[1],3])
        ptd_min=np.min(ptd3d[jj,:,:])
        #print(np.shape(ptd3d[jj,:,:]),np.shape(ons),ons*ptd_min)
        ptd=np.subtract(ptd3d[jj,:,:],ons*ptd_min)
