...
class GenData():
    ...
    def GenWave(self,numSubWave=10):
...
W1 = my_weigh(([784,50]))
b1 = my_bias(([50]))
r1_f=tf.matmul(x,W1)+b1
r1=tf.nn.tanh(r1_f)

W2 = my_weigh(([50,784]))
b2 = my_bias(([784]))
r2_f=tf.matmul(r1,W2)+b2
r2=tf.nn.tanh(r2_f)



kp_prb=tf.placeholder('float')
result=tf.nn.dropout(r2,kp_prb)

ce = tf.reduce_mean(tf.abs(r2-y))
...
b, a = signal.iirdesign([0.01, 0.1], [0.01, 0.12], 2, 40)
