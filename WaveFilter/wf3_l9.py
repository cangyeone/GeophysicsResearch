...
class GenData():
    ...
    def GenWave(self,numSubWave=30):
        ...
        return self.data
    def AddNoise(self,bl=0.5,tp='FIX'):
...
c3=tf.nn.pool(c3_f2,window_shape=[1],pooling_type='AVG',strides=[1],padding='SAME')
"""
c4_w=my_weigh(c4s)
c4_b=my_weigh([c4s[2]])
...
c9=tf.nn.pool(c9_f2,window_shape=[2],pooling_type='AVG',strides=[1],padding='SAME')
"""
...
for itr in range(6000):
    ...
    sess.run(train_step, feed_dict={x: data_noise, y: data,kp_prb:0.6})
    #print(data[0])
...
c3_t=tf.nn.pool(c3_f2_t,window_shape=[1],pooling_type='AVG',strides=[1],padding='SAME')
