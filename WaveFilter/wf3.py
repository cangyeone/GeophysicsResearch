...
class GenData():
    ...
    def GenWave(self,numSubWave=30):
        ...
        sbn=np.random.randint(numSubWave)
...
c1=tf.nn.pool(c1_f2,window_shape=[3],pooling_type='AVG',strides=[1],padding='SAME')
...
data2=tf.reshape(c3,[50,N*c3s[2]])
"""
...
batch = GenData([50,TN])
data=batch.GenWave(15)
data_noise=batch.AddNoise(0.2)
#============================================================================
...
c1_t=tf.nn.pool(c1_f2_t,window_shape=[4],pooling_type='AVG',strides=[1],padding='SAME')

c2_f1_t=tf.nn.conv1d(c1_t,filters=c2_w,stride=1, padding='SAME')+c2_b
c2_f2_t=tf.nn.tanh(c2_f1_t)
c2_t=tf.nn.pool(c2_f2_t,window_shape=[2],pooling_type='AVG',strides=[1],padding='SAME')

c3_f1_t=tf.nn.conv1d(c2_t,filters=c3_w,stride=1, padding='SAME')+c3_b
c3_f2_t=tf.nn.tanh(c3_f1_t)
c3_t=tf.nn.pool(c3_f2_t,window_shape=[2],pooling_type='AVG',strides=[1],padding='SAME')

data_t=tf.reshape(c3_t,[50,TN*c3s[2]])
kp_prb=tf.placeholder('float')
result_t=tf.nn.dropout(data_t,kp_prb)
#===========================================================================
outdata=sess.run(result_t, feed_dict={x_t: data_noise,kp_prb:1})
