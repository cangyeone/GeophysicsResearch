...
for itr in range(1000):
    ...
    sess.run(train_step, feed_dict={x: data_noise, y: data,kp_prb:0.8})
    #print(data[0])
...
"""
kp_prb=tf.placeholder('float')
data2_t=tf.reshape(c3_t,[50,TN])

#===========================================================================
batch2 = GenData([50,TN])
data=batch2.GenWave(15)
data_noise=batch2.AddNoise(0.2)
outdata1=sess.run(data2_t, feed_dict={x_t: data_noise,kp_prb:1,kp_t:1})
outdata2=sess.run(data2_t, feed_dict={x_t: outdata1,kp_prb:1,kp_t:1})
outdata=sess.run(data2_t, feed_dict={x_t: outdata2,kp_prb:1,kp_t:1})
