...
class FilterTrain():
    ...
    def __init__(self,func,raw_data):
        ...
        c1=tf.nn.pool(c1_f2,window_shape=[pool_window[0]],pooling_type=pool_type,strides=[1],padding='SAME')
        ...
        c2=tf.nn.pool(c2_f2,window_shape=[pool_window[1]],pooling_type=pool_type,strides=[1],padding='SAME')
        ...
        c3=tf.nn.pool(c3_f2,window_shape=[pool_window[2]],pooling_type=pool_type,strides=[1],padding='SAME')
        ...
        for itr in range(2000):
            ...
            self.sess.run(train_step, feed_dict={x: data_noise, y: data,kp_prb:0.8})
            #print(data[0])
            #plt.plot(data_noise[0])
            if(itr%100==0):
                print("%d:%f"%(itr,self.sess.run(ce, feed_dict={x: data_noise, y: data,kp_prb:1.})))
        ...
        c1_t=tf.nn.pool(c1_f2_t,window_shape=[pool_window[0]],pooling_type=pool_type,strides=[1],padding='SAME')
        ...
        c2_t=tf.nn.pool(c2_f2_t,window_shape=[pool_window[1]],pooling_type=pool_type,strides=[1],padding='SAME')
        ...
        c3_t=tf.nn.pool(c3_f2_t,window_shape=[pool_window[2]],pooling_type=pool_type,strides=[1],padding='SAME')
...
cvl1=(np.transpose(np.reshape(flt2.sess.run(flt2.c1_w.value()),[4,4])))
plt.figure(1)
plt.subplot(511)
...
plt.plot(MovAvg(data_noise[3],10),color='cornflowerblue',lw=3)
