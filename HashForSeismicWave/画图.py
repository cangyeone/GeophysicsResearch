...
class SacFig():
    ...
    def __init__(self,file_name):
        ...
        for temp_data in self.sac_st[0:1]:
            ...
            x=np.linspace(0,len(temp_data),len(fqData))
            y=np.linspace(0,1,self.fqRspN)
            X,Y = np.meshgrid(x, y)
            plt.figure(1)
            plt.contourf(X,Y,np.log(np.transpose(fqData)))
            plt.xlabel("time(s)")
            plt.ylabel("frquence(Hz)")
            mind=np.min(temp_data)
            rfd=temp_data-mind
            maxd=np.max(rfd)
            rfd=rfd/maxd*0.5+0.25
            #plt.figure(2)
            plt.plot(rfd,c='k')
            plt.xlabel("time(s)")
            plt.ylabel("H(nT)")
            ...
            x=np.linspace(0,1,self.wlRspN)
            y=np.linspace(0,1,self.fqRspN)
            X,Y = np.meshgrid(x, y)
            plt.figure(3)
            plt.matshow((np.reshape(self.wlData[100],[self.fqRspN,self.wlRspN])),cmap=plt.get_cmap('seismic'))
            ...
            plt.figure(77)
            xl=np.linspace(2000,4000,2000)
            plt.plot(xl,temp_data[2000:4000],c='k')
            ct=0
            #plt.text(3000,160,str(self.hash[0])+'\n'+str(self.hash[1])+'\n'+str(self.hash[2])+'\n'+str(self.hash[3]))
            for hsh in self.hash:
                ct=ct+1
                tm=ct*self.fqLagN*self.wlLagN
                if(tm>2000 and tm<4000):
                    plt.text(tm,temp_data[tm],str(hsh[0])+'\n'+str(hsh[1])+'\n'+str(hsh[2])+'\n'+str(hsh[3]))
            plt.xlabel("time(s)")
            plt.ylabel("H(nT)")  
            plt.figure(88)
            xl=np.linspace(8000,10000,2000)
            plt.plot(xl,temp_data[8000:10000],c='k')
            ct=0
            #plt.text(3000,160,str(self.hash[0])+'\n'+str(self.hash[1])+'\n'+str(self.hash[2])+'\n'+str(self.hash[3]))
            for hsh in self.hash:
                ct=ct+1
                tm=ct*self.fqLagN*self.wlLagN
                if(tm>8000 and tm<10000):
                    plt.text(tm,temp_data[tm],str(hsh[0])+'\n'+str(hsh[1])+'\n'+str(hsh[2])+'\n'+str(hsh[3]))
            plt.xlabel("time(s)")
            plt.ylabel("H(nT)")
        plt.show()
...
if __name__ == '__main__':
    ...
    import time
    scFile=GetSecFile()
    for fileName in scFile[0:1]:
        st=time.clock()
        a1=SacFig(fileName)
        outFile=open("tt"+"_hash_file.txt","w")
        sc=0
        hdT=a1.hash
        #gdt=MergeFile(hdT)
        #hd=gdt.GetData()
