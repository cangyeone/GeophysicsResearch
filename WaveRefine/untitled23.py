...
class GenData():
    ...
    def GenWave(self,numSubWave=30):
        ...
        self.data=np.multiply(np.divide(self.data,mx),0.6)
...
class FilterTrain():
    ...
    def my_bias(self,shape):
        ...
        return tf.Variable(init)
    def pool(c1_f2,ws,pt,st):
        return tf.nn.pool(c1_f2,window_shape=ws,pooling_type=py,strides=st,padding='SAME')
    def actv(self,dt):
        return tf.nn.relu(dt)
    def __init__(self,func,raw_data):
        ...
        for itr in pool_stride:
            ctm=ctm*itr
        csrt=1
        for itr in conv_stride:
            csrt=csrt*itr
        x = tf.placeholder(tf.float32,[50,N])
        y = tf.placeholder(tf.float32,[50,int(N/ctm/csrt)*c3s[2]])
        ...
        c1_f1=tf.nn.conv1d(data1,filters=c1_w,stride=conv_stride[0], padding='SAME')+c1_b
        c1_f2=self.actv(c1_f1)
        ...
        c2_f1=tf.nn.conv1d(c1,filters=c2_w,stride=conv_stride[1], padding='SAME')+c2_b
        c2_f2=self.actv(c2_f1)
        ...
        c3_f1=tf.nn.conv1d(c2,filters=c3_w,stride=conv_stride[2], padding='SAME')+c3_b
        c3_f2=self.actv(c3_f1)
        ...
        """
        data2=tf.reshape(c3,[50,int(N/ctm/csrt)*c3s[2]])
        
        ce = tf.reduce_mean(tf.square(data2-y))
        
        train_step = tf.train.AdamOptimizer(0.01).minimize(ce)
        ...
        for itr in range(500):
            ...
            self.sess.run(train_step, feed_dict={x: data_noise, y: sig.resample(data,int(N/ctm/csrt)*c3s[2],axis=1),kp_prb:0.8})
            #print(data[0])
            #plt.plot(data_noise[0])
            if(itr%100==0):
                print("%d:%f"%(itr,self.sess.run(ce, feed_dict={x: data_noise, y:sig.resample(data,int(N/ctm/csrt)*c3s[2],axis=1),kp_prb:1.})))
        ...
        c1_f1_t=tf.nn.conv1d(data_t,filters=c1_w,stride=conv_stride[0], padding='SAME')+c1_b
        c1_f2_t=self.actv(c1_f1_t)
        c1_t=tf.nn.pool(c1_f2_t,window_shape=[pool_window[0]],pooling_type=pool_type,strides=[pool_stride[0]],padding='SAME')
        
        c2_f1_t=tf.nn.conv1d(c1_t,filters=c2_w,stride=conv_stride[1], padding='SAME')+c2_b
        c2_f2_t=self.actv(c2_f1_t)
        ...
        c3_f1_t=tf.nn.conv1d(c2_t,filters=c3_w,stride=conv_stride[2], padding='SAME')+c3_b
        c3_f2_t=self.actv(c3_f1_t)
...
rtData[0][40000:40030]=rtData[0][40000:40030]+np.ones([30])*0.8
rtData[1][40000:40030]=rtData[1][40000:40030]+np.ones([30])*0.8
rtData[2][40000:40030]=rtData[2][40000:40030]+np.ones([30])*0.8
#======================================


batch2 = GenData([50,TN])
data=batch2.GenWave(5)
rtData=batch2.AddNoise(0.2)
