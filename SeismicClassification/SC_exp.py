...
class Classfication():
    ...
    def FitFunc(self,x,a,b,c,d,e,f):
        ...
        return re
    def __init__(self,fileName,valiName,layer=[2,2],kpbfeedt=[1,1,1]):
        
        fcl=self.LayerToFcl(layer)
        kpbfeed=np.ones(np.shape(layer))
        kpbfeed[:len(kpbfeed)]=kpbfeed
        x=tf.placeholder(tf.float32,[None,fcl[0][0]])
        y=tf.placeholder(tf.float32,[None, 2])

        kpb=tf.placeholder(tf.float32,[len(kpbfeed)])
        fc_w=[]
        fc_b=[]
        fc_fn=[]
        fc_d=[]
        ...
        for itr in range(len(fcl)):
            ...
            if(itr==0):
                fc_d.append(tf.nn.dropout(x,kpb[itr]))
                fc_fn.append(tf.matmul(fc_d[itr],fc_w[itr])+fc_b[itr])
            else:
                fc_d.append(tf.nn.dropout(fc[itr-1],kpb[itr]))
                fc_fn.append(tf.matmul(fc_d[itr],fc_w[itr])+fc_b[itr])
            fc.append(self.my_actv(fc_fn[itr]))
        nFcl=len(fcl)

        ce=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=fc[nFcl-1],logits=fc[nFcl-1]))
        ...
        #summary_writer = tf('tmp/logs', graph_def=sess.graph_def)
        st=time.clock()
        for itr in range(3000):
            rd,rv=self.GenTrainData(train_data,train_vali,train_idx,30)
            sess.run(ts,feed_dict={x:rd,y:rv,kpb:kpbfeed})
            if(itr%100==0):
                self.acc=sess.run(accuracy,feed_dict={x:data,y:vali,kpb:kpbfeed})
                ed=time.clock()
        ...
        try:
            ...
            for aa in [[x*0.01,y*0.01] for x in range(10,90) for y in range(10,90)]:
                mat.append(aa)   
            rmat=sess.run(fc[nFcl-1],feed_dict={x:mat,kpb:kpbfeed})
            ...
            def PlotWeigh(www,layer):
                ...
                for idy in range(len(www)):
                    for idx in range(len(www[0])):
                        plt.plot([layer-1,layer],[idx,idy],c='y',lw=www[idy][idx]*3/mx,alpha=www[idy][idx]*2)
                        plt.scatter([layer-1],[idx])
                        plt.scatter([layer],[idy])
                    plt.plot([layer,layer],[idx,idy],c='b',linestyle='--',lw=3,alpha=0.7)
            for itr in range(nFcl):
...
if __name__ == '__main__':
    ...
    layer=[2,4,4,3,2]
    #读取文件名
    fileName=r'中国数据.xlsx'
    valiFileName=r'中国数据.xlsx'
    #训练
